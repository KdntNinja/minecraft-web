...
type TileRenderJob struct {
	blockType coretypes.BlockType
	px, py    float64
	tile      *ebiten.Image
}

// DrawWithCameraAsync renders the world with camera offset (single-threaded, no concurrency)
func DrawWithCameraAsync(worldChunks map[generation.ChunkCoord]*coretypes.Chunk, screen *ebiten.Image, cameraX, cameraY float64) {
	if !isInitialized {
		initTileImages()
	}
...
func DrawWithCameraCountBlocks(worldChunks map[generation.ChunkCoord]*coretypes.Chunk, screen *ebiten.Image, cameraX, cameraY float64, blockCounter *int) {
	screenWidth, screenHeight := screen.Bounds().Dx(), screen.Bounds().Dy()
	tileSize := settings.TileSize
	chunkWidth := settings.ChunkWidth
	chunkHeight := settings.ChunkHeight
